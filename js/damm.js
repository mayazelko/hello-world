...
var blueWinner = 0;
var purpleWinner = 0;
var AudioWin = new Audio('wingame.wav');
//////////     0=white       1=black     2=blue   3=purple  4=blueCrown  5=pupleCrown
var images = ["white.jpg", "black.jpg", "b.PNG", "b1.PNG", "crown2.PNG", "crown1.PNG"];
var damka =
	[
		[0, 2, 0, 2, 0, 2, 0, 2],
		[2, 0, 2, 0, 2, 0, 2, 0],
...
function BuildBoard() {
	...
	for (sh = 0; sh < damka.length; sh++) {
		...
		for (am = 0; am < damka.length; am++) {
			textImgTag = "<img src=" + images[damka[sh][am]] + " style='widows:70px; height:70px' />";
			text = text + "<td id='" + num + "' onclick='SwitchPic(this);' style = width:70px; height:70px >" + textImgTag + "</td>";
			num++;
		}
		text = text + "</tr>";
	}
...
function SwitchPic(currentThis) {
	...
	if (FirstClick) {
		if (damka[CurrentRow][CurrentCol] == 2 && turn == true || damka[CurrentRow][CurrentCol] == 4 && turn == true) {
			document.getElementById("ss").innerHTML = ("purple's turn");
			firstThis = currentThis;
			FirstClick = !FirstClick;
			turn = false;
		}
		else if (damka[CurrentRow][CurrentCol] == 3 && turn == false || damka[CurrentRow][CurrentCol] == 5 && turn == false) {
			document.getElementById("ss").innerHTML = ("blue's turn");
			firstThis = currentThis;
			FirstClick = !FirstClick;
			turn = true;
		}
